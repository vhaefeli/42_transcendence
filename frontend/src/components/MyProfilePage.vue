  ...
  <div id="profile-container">
    ...
    <section class="ft-container">
      ...
      >
        ...
        ></div>
        <div class="ft-connection-circle" id="current-profile-pic">
          <StatusBubble :status="user.status"></StatusBubble>
        </div>
        ...
        <div class="ft-tab-content ft-bg-color-profile" id="buttons-container">
          <a
            ...
            class="t-btn-pink ft-color-edit ft-my-profile ft-icon-small icon-btn-cursor"
            id="edit"
            ><img
              src="../assets/icons/user-pen-solid.svg"
              alt="edit my profile"
          /></a>
        </div>
      </div>
      ...
      >
        ...
        <div
          ...
          class="ft-tab-content ft-border-color-profile ft-tab-border text-left ft-scrollable"
        >
          <ul>
            <div v-if="invites">
      ...
      <div
        class="flex flex-col text-center ft-right-tab ft-my-profile"
        id="sent-requests"
        :class="{ foreground: foregroundTab === 'sentRequests' }"
        @click="setForegroundTab('sentRequests')"
      >
        <div class="ft-tab-folder ft-tab-title ft-bb-color-profile">
          Sent requests
        </div>
        <div
          id="sentRequestsScroll"
          class="ft-tab-content ft-border-color-profile ft-tab-border text-left ft-scrollable"
        >
          <ul>
            <div v-if="invitesSent">
              <div v-if="invitesSent.length === 0">
                <EmptyText :text="`No pending request!`" :white="false" />
              </div>
      ...
      <div
        class="flex flex-col text-center ft-left-tab ft-my-profile"
        id="friends-list"
        :class="{ foreground: foregroundTab === 'friends' }"
        @click="setForegroundTab('friends')"
      >
        <div class="ft-tab-folder ft-tab-title ft-bb-color-profile">
          Friends
        </div>
        ...
        >
          <ul>
            <div v-if="friends.length === 0">
              <EmptyText
                :text="'You have no friends... Too bad!'"
                :white="false"
              />
            </div>
      ...
      <div
        class="flex flex-col text-center ft-left-tab ft-my-profile"
        id="friends-search"
        :class="{ foreground: foregroundTab === 'search' }"
        @click="setForegroundTab('search')"
      >
        <div class="ft-tab-folder ft-tab-title ft-bb-color-profile">
          Add a new friend
        </div>
        <div
          class="ft-tab-content ft-border-color-profile ft-tab-border text-left"
        >
          <div class="flex flex-row justify-center">
            ...
            <div :class="{ 'cursor-not-allowed': !searchSelectedUserId }">
              <a
                ...
                class="t-btn-pink ft-color-add ft-icon-small icon-btn-size icon-btn-cursor"
                :class="{
                  'opacity-50 searchan-noClick': !searchSelectedUserId,
                }"
              >
                <img
                  src="../assets/icons/user-plus-solid.svg"
                  alt="send a friend request"
                  title="send them a friend request"
                />
              </a>
            </div>
          </div>
          ...
          <ul>
            <div v-if="blocked">
              <div v-if="blocked.length === 0">
                <EmptyText :text="`You didn't block anybody`" :white="true" />
              </div>
