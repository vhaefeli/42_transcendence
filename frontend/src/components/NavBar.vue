<template>
  <nav id="ft-main-nav" class="w-full flex justify-center items-end">
      ...
      <div id="ft-nav-profile">
        <div id="ft-nav-profile-username">
          <div class="cursor-pointer" @click="toggle">
            <p class="truncate" style="max-width: 10rem">
              {{ props.userStore.user.username }}
            </p>
            <div
              v-if="activeLogout"
              id="ft-logout-btn"
              class="bg-white p-1 w-full flex items-center justify-center"
            >
              <router-link :to="'/login?logout=true'">Logout</router-link>
            </div>
          </div>
        </div>
        <div
          class="ft-profile-pic"
          id="ft-nav-profile-img"
          :style="{
            background: 'url(' + props.userStore.user.avatar_url + ')',
          }"
        ></div>
      </div>
    </div>
    <div v-else>
      <div
        id="ft-logout-btn"
        class="w-fit absolute top-4 right-4 p-1 flex items-center justify-center"
      >
        <router-link :to="'/login?logout=true'">Logout</router-link>
      </div>
    </div>
...
<style scoped>
  ...
  #ft-logout-btn {
    height: 0;
    overflow: hidden;
    animation: appearDown 0.5s forwards;
  }
